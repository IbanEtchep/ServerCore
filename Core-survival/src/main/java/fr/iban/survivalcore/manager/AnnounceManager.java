...
public class AnnounceManager {
    ...
    public void sendAnnounce(Player player, String message) {
        ...
        if (plugin.getEconomy().getBalance(player) >= 250) {
            ...
            messagingManager.sendMessage("SYNC_ANNOUNCE_COOLDOWN_CHANNEL", uuid.toString());
        } else {
    ...
    public boolean hasCooldown(Player player) {
        if (cooldowns.containsKey(player.getUniqueId())) {
            ...
            long secondsLeft = ((lastUsage / 1000) + cooldownTime) - (System.currentTimeMillis() / 1000);
            if (secondsLeft > 0) {
                player.sendMessage("§cVous pourrez à nouveau faire ça dans " + DateUtil.formatDateDiff(lastUsage + cooldownTime * 1000) + ".");
